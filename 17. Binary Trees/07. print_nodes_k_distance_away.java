...
public class Main {
    ...
    public static ArrayList < Node > nodeToRootPath(Node node, int data) {
        ...
        ArrayList < Node > n2lcp = nodeToRootPath(node.left, data); //node to left child path

        if (n2lcp.size() > 0) {
            n2lcp.add(node); //converting n2lcp to node to root path
            return n2lcp;
        }

        ArrayList < Node > n2rcp = nodeToRootPath(node.right, data); //node to right child path

        if (n2rcp.size() > 0) {
            n2rcp.add(node); //converting n2rcp to node to root path
            return n2rcp;
        }
